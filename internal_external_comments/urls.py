...
urlpatterns = [
    ...
    url(r'^create/(?P<app_label>[\w-]+)/(?P<model>[\w-]+)/(?P<object_pk>[0-9]+)/$', views.CommentObjectCreateView.as_view(), name='comments-object-create'),
    ...
    url(r'^list/(?P<app_label>[\w-]+)/(?P<model>[\w-]+)/(?P<object_pk>[0-9]+)/$',
